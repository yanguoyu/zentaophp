...
class approveModel extends model
{
    ...
    public function setMenu($projects, $projectID, $buildID = 0, $extra = '')
    {
        ...
        else
        {
            ...
            {
                if(common::hasPriv('approve', 'create')) $projectIndex .= '<li>' . html::a(helper::createLink('approve', 'create'), '<i class="icon icon-plus"></i> ' . $this->lang->approve->create) . '</li>';
            }
    ...
    public function tree()
    {
        ...
        foreach($productGroup as $productID => $projects)
        {
            ...
            $productName  = isset($products[$productID]) ? $products[$productID] : $this->lang->approve->noProduct;
            ...
            if($hasDone)
            {
                $projectTree .= "<li>{$this->lang->approve->selectGroup->done}<ul>";
                foreach($projects as $project)
                {
                    if($project->status == 'done' or $project->status == 'closed')
                    {
    ...
    {
        return $this->dao->select('*')->from(TABLE_APPROVE)->where('id')->in($projectIDList)->fetchAll('id');
    }
    ...
    public function getInvolvedList($status = 'involved', $limit = 0, $productID = 0, $branch = 0)
    {
        ...
        {
            return $this->dao->select('t1.*, IF(INSTR(" done,closed", t1.status) < 2, 0, 1) AS isDone')->from(TABLE_APPROVE)->alias('t1')
    ...
    {
        $projects = $this->dao->select('*')->from(TABLE_APPROVE)
    ...
    public function getRelatedProjects($projectID)
    {
        ...
        return $this->dao->select('t1.id, t1.name')->from(TABLE_APPROVE)->alias('t1')
    ...
    {
        return $this->dao->select('id, name')->from(TABLE_APPROVE)->where('parent')->eq((int)$projectID)->fetchPairs();
    }
    ...
    {
        $sql = "UPDATE " . TABLE_APPROVE . " SET parent = 0 WHERE parent = '$projectID'";
        $this->dbh->exec($sql);
        if(!isset($_POST['childs'])) return;
        $childs = array_unique($_POST['childs']);
        foreach($childs as $childProjectID)
        {
            $sql = "UPDATE " . TABLE_APPROVE . " SET parent = '$projectID' WHERE id = '$childProjectID'";
            $this->dbh->query($sql);
        }
    }
    ...
    public function linkStories($projectID)
    {
        ...
        if($count != 0) echo js::alert(sprintf($this->lang->approve->haveDraft, $count)) . js::locate(helper::createLink('project', 'create', "projectID=$projectID"));
    }
    ...
    public function getTeams2Import($account, $currentProject)
    {
        return $this->dao->select('t1.root, t2.name as projectName')
            ...
            ->leftJoin(TABLE_APPROVE)->alias('t2')->on('t1.root = t2.id')
    ...
    public function computeBurn()
    {
        ...
        $projects = $this->dao->select('id, code')->from(TABLE_APPROVE)
            ->where("end")->ge($today)
    ...
    public function summary($tasks)
    {
        ...
        return sprintf($this->lang->approve->taskSummary, $taskSum, $statusWait, $statusDoing, $totalEstimate, round($totalConsumed, 1), round($totalLeft, 1));
    }
    ...
    {
        $projects = $this->dao->select('id,`order`')->from(TABLE_APPROVE)->orderBy('order')->fetchPairs('id', 'order');
        ...
        foreach($projects as $id => $order)
        {
            ...
            $this->dao->update(TABLE_APPROVE)->set('`order`')->eq($newOrder)->where('id')->eq($id)->exec();
        }
    }
    ...
    public function buildTaskSearchForm($projectID, $projects, $queryID, $actionURL)
    {
        ...
        $this->config->project->search['params']['project']['values'] = array(''=>'', $projectID => $projects[$projectID], 'all' => $this->lang->approve->allProject);
