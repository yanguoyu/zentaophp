...
<div id="mainMenu" class="clearfix table-row">
  <div class="btn-toolbar pull-left">
    ...
    <div class="pull-left table-group-btns">
      <button type="button" class="btn btn-link group-collapse-all"><?php echo $lang->approve->treeLevel['root'];?> <i class="icon-fold-all"></i></button>
      <button type="button" class="btn btn-link group-expand-all"><?php echo $lang->approve->treeLevel['all'];?> <i class="icon-unfold-all"></i></button>
    </div>
    <?php endif;?>
    <?php if(isset($lang->approve->groupFilter[$groupBy])):?>
    <?php foreach($lang->approve->groupFilter[$groupBy] as $filterKey => $name):?>
    <?php
    $active = '';
    ...
    <?php endforeach;?>
    <?php else:?>
    <?php echo html::a(inlink('grouptask', "projectID=$projectID&groupBy=$groupBy"), "<span class='text'>{$lang->approve->allTasks}</span> <span class='label label-light label-badge'>{$allCount}</span>", '', "class='btn btn-link btn-active-text'");?>
    <?php endif;?>
  </div>
  <div class="btn-toolbar pull-right">
    ...
    <div class="btn-group">
      ...
      <ul class="dropdown-menu">
        ...
        echo "<li $class>" . html::a($link, $lang->approve->importTask, '', $misc) . "</li>";
        ...
        echo "<li $class>" . html::a($link, $lang->approve->importBug, '', $misc) . "</li>";
        ?>
      </ul>
    </div>
    <?php
        ...
        <th class="c-side text-left has-btn group-menu">
          <div class="dropdown">
            <a href="" data-toggle="dropdown" class="btn text-left btn-block btn-link clearfix">
              <span class='pull-left'><?php echo zget($lang->approve->groups, $groupBy, null);?></span>
              <i class="icon icon-caret-down hl-primary text-primary pull-right"></i>
            </a>
            <ul class="dropdown-menu">
              <?php foreach($lang->approve->groups as $key => $value):?>
              <?php
    ...
    <tbody>
      ...
      <tr data-id='<?php echo $groupIndex?>' <?php if($groupIndex > 1 and $i == 0) echo "class='divider-top'";?>>
        ...
        <td rowspan='<?php echo $groupSum?>' class='c-side text-left group-toggle text-top<?php if($groupSum > 4) echo ' c-side-lg'?>'>
          <div class='group-header'>
            ...
            <?php if($groupBy == 'assignedTo' and isset($members[$task->assignedTo])) printf($lang->approve->memberHoursAB, zget($users, $task->assignedTo), $members[$task->assignedTo]->totalHours);?>
            <?php printf($lang->approve->groupSummaryAB, $groupSum, $groupWait, $groupDoing, $groupEstimate, $groupConsumed, $groupLeft);?>
            </div>
          </div>
        </td>
        <?php endif;?>
      ...
      <tr class='group-toggle group-summary hidden <?php if($groupIndex > 1) echo 'divider-top';?>' data-id='<?php echo $groupIndex?>'>
        ...
        <td colspan='13'>
          ...
          <?php if($groupBy == 'assignedTo' and isset($members[$task->assignedTo])) printf($lang->approve->memberHours, zget($users, $task->assignedTo), $members[$task->assignedTo]->totalHours);?>
          <?php printf($lang->approve->countSummary, $groupSum, $groupDoing, $groupWait);?>
          <?php printf($lang->approve->timeSummary, $groupEstimate, $groupConsumed, $groupLeft);?>
          </div>
        </td>
      </tr>
      <?php endif;?>
