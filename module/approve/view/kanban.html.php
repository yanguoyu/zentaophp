...
<div id='mainMenu' class='clearfix'>
  <div class='btn-toolbar pull-right'>
    <?php echo html::a($this->createLink('project', 'ajaxKanbanSetting', "projectID=$projectID"), "<i class='icon-cog muted'></i> " . $lang->approve->kanbanSetting, '', "class='iframe btn btn-link'");?>
    <?php if(common::hasPriv('project', 'printKanban')) echo html::a($this->createLink('project', 'printKanban', "projectID=$projectID"), "<i class='icon-printer muted'></i> " . $lang->approve->printKanban, '', "class='iframe btn btn-link' id='printKanban' title='{$lang->approve->printKanban}' data-width='500'");?>
    <?php
    ...
    <div class='btn-group'>
      ...
      <ul class='dropdown-menu' id='importActionMenu'>
        ...
        echo "<li $misc>" . html::a($link, $lang->approve->importTask, '', $misc) . "</li>";
        ...
        echo "<li $misc>" . html::a($link, $lang->approve->importBug, '', $misc) . "</li>";
        ?>
      </ul>
    </div>
    <?php
    ...
    <thead>
      <tr>
        ...
        <th class="c-board c-side has-btn">
          <div class="dropdown">
            <?php $dropTitle = $type == 'story' ? $lang->approve->orderList[$storyOrder] : $lang->task->$type;?>
            ...
            <ul class='dropdown-menu text-left'>
              <?php foreach($lang->approve->orderList as $key => $value):?>
              ...
              <?php endforeach;?>
              <?php echo "<li" . ($type == 'assignedTo' ? " class='active'" : '') . ">" . html::a(inlink('kanban', "project=$projectID&type=assignedTo"), $lang->approve->groups['assignedTo']) . "</li>";?>
              <?php echo "<li" . ($type == 'finishedBy' ? " class='active'" : '') . ">" . html::a(inlink('kanban', "project=$projectID&type=finishedBy"), $lang->approve->groups['finishedBy']) . "</li>";?>
            </ul>
          </div>
        </th>
        <?php endif;?>
              ...
              <nav class='board-actions nav nav-default'>
                <li class='dropdown'>
                  ...
                  <ul class='dropdown-menu pull-right'>
                    ...
                    echo (common::hasPriv('task', 'create'))         ? '<li>' . html::a($this->createLink('task', 'create', "projectID=$story->project&storyID=$story->id&moduleID=$story->module", '', true), $lang->approve->wbs, '', $misc) : '' . '</li>';
                    echo (common::hasPriv('task', 'batchCreate'))    ? '<li>' . html::a($this->createLink('task', 'batchCreate', "projectID=$story->project&storyID=$story->id&moduleID=0&taskID=0&iframe=true", '', true), $lang->approve->batchWBS, '', $misc) : '' . '</li>';
                    echo (common::hasPriv('project', 'unlinkStory')) ? '<li>' . html::a($this->createLink('project', 'unlinkStory', "projectID=$story->project&storyID=$story->story&confirm=no", '', true), $lang->approve->unlinkStory, 'hiddenwin') : '' . '</li>';
