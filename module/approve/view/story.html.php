...
<?php js::set('confirmUnlinkStory', $lang->approve->confirmUnlinkStory)?>
<div id="mainMenu" class="clearfix">
  ...
  <div class="btn-toolbar pull-left">
    ...
    if(common::hasPriv('project', 'storykanban')) echo html::a($this->createLink('project', 'storykanban', "projectID=$project->id"), "<span class='text'>{$lang->approve->kanban}</span>", '', "class='btn btn-link'");
    ?>
  ...
  <div class="btn-toolbar pull-right">
    ...
    $this->lang->story->create = $this->lang->approve->createStory;
    ...
    if(commonModel::isTutorialMode())
    {
        ...
        echo html::a($this->createLink('tutorial', 'wizard', "module=project&method=linkStory&params=$wizardParams"), "<i class='icon-link'></i> {$lang->approve->linkStory}",'', "class='btn btn-link link-story-btn'");
    }
    else
    {
        echo "<div class='btn-group dropdown-hover'>";
        echo "<button type='button' class='btn btn-primary' id='linkButton'>";
        echo "<i class='icon-link'></i> {$lang->approve->linkStory} <span class='caret'></span>";
        echo '</button>';
        echo "<ul class='dropdown-menu pull-right' id='linkActionMenu'>";
        if(common::hasPriv('project', 'linkStory')) echo '<li>' . html::a(inlink('linkStory', "project=$project->id"), $lang->approve->linkStory). "</li>";
        if(common::hasPriv('project', 'importPlanStories')) echo '<li>' . html::a('#linkStoryByPlan', $lang->approve->linkStoryByPlan, '', 'data-toggle="modal"') . "</li>";
        echo '</ul>';
        echo '</div>';
    }
    ?>
  ...
  <div class="main-col">
    ...
    <div class="table-empty-tip">
      <p>
        ...
        <?php echo html::a($this->createLink('project', 'linkStory', "project=$project->id"), "<i class='icon icon-link'></i> " . $lang->approve->linkStory, '', "class='btn btn-info'");?>
        <?php endif;?>
      </p>
    </div>
    <?php else:?>
      ...
      <table class='table tablesorter has-sort-head' id='storyList'>
        <thead>
          ...
          <?php $vars = "projectID={$project->id}&orderBy=%s&type=$type&param=$param&recTotal={$pager->recTotal}&recPerPage={$pager->recPerPage}"; ?>
            ...
            <?php if($canOrder):?>
            <th class='w-60px c-sort {sorter:false}'> <?php common::printOrderLink('order',      $orderBy, $vars, $lang->approve->orderAB);?></th>
            <?php endif;?>
        ...
        <tbody id='storyTableList' class='sortable'>
          ...
          <tr id="story<?php echo $story->id;?>" data-id='<?php echo $story->id;?>' data-order='<?php echo $story->order ?>' data-estimate='<?php echo $story->estimate?>' data-cases='<?php echo zget($storyCases, $story->id, 0)?>'>
            ...
            <td class='c-actions'>
              ...
              $lang->task->create = $lang->approve->wbs;
              if(commonModel::isTutorialMode())
              {
                  $wizardParams = helper::safe64Encode($param);
                  echo html::a($this->createLink('tutorial', 'wizard', "module=task&method=create&params=$wizardParams"), "<i class='icon-plus'></i>",'', "class='btn btn-task-create' title='{$lang->approve->wbs}'");
              }
              else
              {
              ...
              $lang->task->batchCreate = $lang->approve->batchWBS;
              ...
              if(common::hasPriv('project', 'unlinkStory', $project))
              {
                  ...
                  echo html::a("javascript:ajaxDelete(\"$unlinkURL\", \"storyList\", confirmUnlinkStory)", '<i class="icon-unlink"></i>', '', "class='btn' title='{$lang->approve->unlinkStory}'");
              }
              ?>
            </td>
          </tr>
      ...
      <div class='table-footer'>
        ...
        <div class='table-actions btn-toolbar'>
          ...
          if(common::hasPriv('project', 'batchUnlinkStory'))
          {
              ...
              echo html::commonButton($lang->approve->unlinkStoryAB, "data-form-action='$actionLink'");
          }
          ?>
        </div>
  ...
  <div class="modal-dialog mw-500px">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title"><?php echo $lang->approve->linkStoryByPlan;?></h4><?php echo '(' . $lang->approve->linkStoryByPlanTips . ')';?>
      </div>
      <div class="modal-body">
        <div class='input-group'>
          <?php echo html::select('plan', $allPlans, '', "class='form-control chosen' id='plan'");?>
          <span class='input-group-btn'><?php echo html::commonButton($lang->approve->linkStory, "id='toTaskButton'", 'btn btn-primary');?></span>
        </div>
      </div>
    </div>
  </div>
