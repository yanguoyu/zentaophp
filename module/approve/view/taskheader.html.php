<div id='featurebar'>
  <ul class='nav'>
    ...
    echo "<li id='kanbanTab'>"; common::printLink('project', 'kanban', "projectID=$projectID", $lang->approve->kanban); echo '</li>';
    if($project->type == 'sprint' or $project->type == 'waterfall')
    {
        echo "<li id='burnTab'>";
        common::printLink('project', 'burn', "project=$projectID", $lang->approve->burn);
        echo '</li>';
    }
    echo "<li id='treeTab'>"; common::printLink('project', 'tree', "projectID=$projectID", $lang->approve->tree); echo '</li>';
    echo "<li id='groupTab' class='dropdown'>";
    $groupBy = isset($groupBy) ? $groupBy : '';
    $current = zget($lang->approve->groups, isset($groupBy) ? $groupBy : '', '');
    echo html::a('javascript:;', $current . " <span class='caret'></span>", '', "data-toggle='dropdown'");
    echo "<ul class='dropdown-menu'>";
    foreach ($lang->approve->groups as $key => $value)
    {
        if($key == '') continue;
    ...
    echo '</ul></li>';
    if($this->methodName === 'task') echo "<li id='bysearchTab'><a href='#'><i class='icon-search icon'></i>&nbsp;{$lang->approve->byQuery}</a></li> ";
    ?>
  </ul>
  <div class='actions'>
    <div class='btn-group'>
      ...
      <div class='btn-group'>
        ...
        echo "<li>" . html::a($link, $lang->approve->importTask, '', $misc) . "</li>";
        ...
        echo "<li>" . html::a($link, $lang->approve->importBug, '', $misc) . "</li>";
        ?>
        </ul>
      </div>
    </div>
